...
**Ordering intent:** #1 → #6 → #7 → #2 → (#3 ∥ #4) → #5, mirroring ROADMAP's polish → content → ship. #6 was added 2026-08-04 (brainstormed, not triaged from inbox) and belongs in the polish block with #1. #7 was added 2026-08-04 (brainstormed, not triaged from inbox); it touches `completion-actions.tsx`, so it is not parallel-safe with #1 (US-07) — sequence it in the polish block. #8 was added 2026-08-04 (brainstormed, not triaged from inbox; offline merge proven same day — see spec CONTEXT); it also touches `completion-actions.tsx`, so it sequences after #1 and #7, and its ARCHITECTURE §5 revision + pregen output should land before #2's budget pass measures final assets.
