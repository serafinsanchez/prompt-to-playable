...
- Failure at any stage: halt pipeline, surface Meshy's `task_error` verbatim + note that failed tasks auto-refund (a DevEx teaching moment). Retry = re-run stage via `retryFailedStage(run)` (pure; resets only the failed stage to pending and the run to `running`, so `createPipeline({run})` resumes it); upstream completed stages are reusable via stored task ids. Always a user click — never automatic.
...
### 2026-08-03 — Live-run hardening: poll-failure cap, keyless-resume guard, thumbnail_url rail
- **Chose:** (1) the pipeline machine fails the active stage after `MAX_CONSECUTIVE_TICK_FAILURES` (5) consecutive unexpected tick errors — a resumed run whose task ids Meshy no longer holds (retention ~3 days) otherwise 404-polls forever with no retry/start-over affordance; success or either 429 flavor resets the count. (2) `hydrate()` restores a running run without starting the ticker when sessionStorage holds no key — polling keyless manufactured a "Key rejected" the visitor never earned; `setKey()` resumes via the existing `resumeTicker()` guard. (3) `StageState.thumbnailUrl?` (optional — pre-existing snapshots lack it) persists Meshy's `thumbnail_url`; the stage rail renders it in a plain `<img>` (no CORS check applies to image display) and only falls back to the GLB-snapshot renderer when absent, cutting the rail's live-run cost from ~66 MB of raw meshes to three small PNGs. The fallback path now also disposes material textures (Material.dispose() does not).
- **Considered:** always-visible "Start over" during running (weaker: leaves the unbounded poll loop); STORAGE_VERSION bump for the new field (unnecessary — additive and optional).
- **Reversibility:** easy. — **Related:** 2026-08-03 asset-proxy entry; live-run audit findings #1, #2, #8, #9.

### 2026-08-03 — Rig-first generation defaults + preview gate
- **Chose:** (1) every preview task sends `pose_mode: "a-pose"` + `topology: "quad"` and every refine sends an anti-lettering `texture_prompt` (constants in `lib/meshy/client.ts`, mirrored by the API panel's derive-don't-duplicate descriptor) — a live basketball-player run produced a mid-dunk pose, duplicated feet, and smudged jersey text, all rig-hostile; (2) the store scaffolds live prompts with character phrasing (`lib/meshy/prompt-craft.ts`) and the prompt bar surfaces advisory action-language/held-prop hints, never blocking; (3) browser machines pass `gatePreview: true` so the run pauses at a new `"awaiting-review"` status after preview succeeds — approve continues, re-roll resets the preview stage while folding its consumed credits into `PipelineRun.discardedCredits` (creditsSpent never understates). Node runners (pregen/spike) never gate — they loop on `"running"` and must not deadlock.
- **Considered:** `symmetry_mode`/`is_a_t_pose` (deprecated no-ops — rejected); silently stripping action words from prompts (rejected: misrepresents the visitor's prompt; hint + scaffold instead); STORAGE_VERSION bump (unnecessary — `previewApproved`/`discardedCredits` are additive-optional, and a v3 run resumed mid-preview just meets the gate late).
- **Reversibility:** easy (constants + one option flag). — **Related:** PRD §5 credit-respect; MESHY_CLAUDE.md deprecated-params gotcha; reference-guide.html text-to-3d parameter table.
